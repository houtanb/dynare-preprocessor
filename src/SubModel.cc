...
OlsModelTable::OlsModelTable(SymbolTable &symbol_table_arg) :
  symbol_table(symbol_table_arg)
{
}

void
OlsModelTable::addOlsModel(string name_arg, string datafile_arg, vector<string> eqtags_arg,
                           vector<string> fitted_eqtags_arg, vector<string> fitted_names_arg,
                           vector<string> fitted_transformation_arg)
{
  if (isExistingOlsModelName(name_arg))
    {
      cerr << "Error: an OLS model already exists with the name " << name_arg << endl;
      exit(EXIT_FAILURE);
    }

  datafiles[name_arg] = move(datafile_arg);
  eqtags[name_arg] = move(eqtags_arg);
  fitted_eqtags[name_arg] = move(fitted_eqtags_arg);
  fitted_names[name_arg] = move(fitted_names_arg);
  fitted_transformation[name_arg] = move(fitted_transformation_arg);
  names.insert(move(name_arg));
}

map<string, vector<string>>
OlsModelTable::getEqTags() const
{
  return eqtags;
}

vector<string>
OlsModelTable::getEqTags(const string &name_arg) const
{
  checkModelName(name_arg);
  return eqtags.find(name_arg)->second;
}
...
void
OlsModelTable::writeOutput(const string &basename, ostream &output) const
{
  for (const auto &name : names)
    {
      string filename = "+" + basename + "/ols_" + name + ".m";
      ofstream ar_output;
      ar_output.open(filename, ios::out | ios::binary);
      if (!ar_output.is_open())
        {
      ...
      ar_output << "function ar = ols_" << name << "()" << endl
                << "%function ar = ols_" << name << "()" << endl
                << "% File automatically generated by the Dynare preprocessor" << endl << endl;
...
void
OlsModelTable::writeJsonOutput(ostream &output) const
{
}

void
OlsModelTable::checkModelName(const string &name_arg) const
{
  if (!isExistingOlsModelName(name_arg))
    {
      cerr << name_arg << " is not a recognized equation tag" << endl;
      exit(EXIT_FAILURE);
    }
}
